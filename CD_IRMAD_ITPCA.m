...
%
%   CD_IRMAD_ITPCA(image_t1,image_t2,method);  * in line at least 3 arguments are needed
%
...
%                         1 to use the principal ocmponents
%                         default value = 0
%
...
%
%   - chi2_itpca_pc2    - 1st method: the Chi Square is obtained using the 2nd principalcomponent
...
%	initial change mask,"
%	IEEE International Geoscience and Remote Sensing Symposium (IGARSS '12), 2012, pp. 6769?6772.
%
...
disp('/////////////////////////////////////////////////');
disp('/////////////////////////////////////////////////');
disp('. . . CD_IRMAD_ITPCA: FUNCTION IN PROGRESS . . .');
disp('/////////////////////////////////////////////////');
...
if size(varargin) == 0
    ...
    switch roc_quest
        case 'Yes'
            ...
            prompt = {'Insert the unchange class value:'};
            dlg_title = 'Unchange class value';
            num_lines = 1;
            def = {'0'};
            options.Resize='on';
            answer = inputdlg(prompt,dlg_title,num_lines,def,options);
            ...
            unchange_val = str2double(answer{1});
...
if strcmp(method,'ITPCA') == 1
    ...
    if isequal(size_im ,0)      % ---------SAVE FILE IN WORKSPACE---------
        ...
        disp('CD_IRMAD_ITPCA - ITPCA: Chi Square computation');
        ...
        if flag_roc==1
            if isequal(dimWin,0)
                ROCeval(file_chi2_pc2,gt,unchange_val,change_val,1,file_ROCdata_pc2,file_CD_pc2);
                ROCeval(file_chi2_diff,gt,unchange_val,change_val,1,file_ROCdata_diff,file_CD_diff);
            end
            if ~isequal(dimWin,0)
                 ROCeval(file_chi2_wrm,gt,unchange_val,change_val,1,file_ROCdata_wrm,file_CD_wrm);
            end
        end
    ...
    disp('/////////////////////////////////////////////////');
    disp('. . . CD_IRMAD_ITPCA - ITPCA: PROCESS OVER . . .');
    disp('/////////////////////////////////////////////////');
...
elseif strcmp(method,'IRMAD') == 1
    ...
    if isequal(size_im ,0)          % ---------SAVE FILE IN WORKSPACE---------
        ...
        if ~isequal(dimWin,0)
            ...
            disp('CD_IRMAD_ITPCA - IRMAD: applying WRM filter');
            wrm_irmad = WRM(file_cv1, file_cv2, dimWin, save_name,path_name);
            
            disp('CD_IRMAD_ITPCA - IRMAD: Chi Square computation');
        ...
        if flag_roc==1
            if isequal(dimWin,0)
                ROCeval(file_chi2,gt,unchange_val,change_val,1,file_ROCdata_irmad,file_CD_irmad);
            end
            if ~isequal(dimWin,0)
                ROCeval(file_chi_irmad_wrm,gt,unchange_val,change_val,1,file_ROCdata_irmad_wrm,file_CD_irmad_wrm);
            end
        end
    ...
    elseif isequal(size_im,1)       % ------------- LINE by LINE-------------
        ...
        if ~isequal(dimWin,0)
            ...
            disp('CD_IRMAD_ITPCA - IRMADLine: applying WRM filter');
    ...
    disp('/////////////////////////////////////////////////');
    disp('. . . CD_IRMAD_ITPCA - IRMAD: PROCESS OVER . . .');
    disp('/////////////////////////////////////////////////');
end
...
GT_new(GT==unchange_val)=0; % unchange
GT_new(GT==change_val)=1; % change

%  ----- get the index of the reference map (0 = unchanges, 1 = changes) -----
GT_new = GT_new(:);
...
%  ----- get the values of the pixels belongin to the reference map -----
